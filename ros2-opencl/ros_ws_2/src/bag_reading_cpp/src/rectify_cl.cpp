#include <bag_reading_cpp/rectify_cl.hpp>
...
RectifyCL::RectifyCL(const rclcpp::NodeOptions &options)
...
size_t RectifyCL::get_msg_size(sensor_msgs::msg::Image::ConstSharedPtr image_msg){
...
size_t RectifyCL::get_msg_size(sensor_msgs::msg::CameraInfo::ConstSharedPtr msg){
...
void RectifyCL::imageCb(sensor_msgs::msg::Image::ConstSharedPtr image_msg,
...
{   
  ...
  TRACEPOINT(
    ...
    assert(error == CL_SUCCESS);
    error = krnl_2.setArg(2, img_matrix);
    assert(error == CL_SUCCESS);
    // error = krnl_2.setArg(0, host_input_cl);
    // assert(error == CL_SUCCESS);
    // error = krnl_2.setArg(1, host_output_cl);
...
void RectifyCL::initializeOpenCL()
{
    ...
    // assert(error == CL_SUCCESS);
    // krnl_1 = k1;
    cl::Kernel k2(program, "rectifyImageBuffer", &error);
    assert(error == CL_SUCCESS);
    krnl_2 = k2;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(image_proc::RectifyCL)
