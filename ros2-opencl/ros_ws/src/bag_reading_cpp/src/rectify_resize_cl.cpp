    ...
    : Node("image_reader_node"), bag_file_path_(bag_file_path)
{
    ...
    timer_ = this->create_wall_timer(
        1ms, std::bind(&CLImageProc::timerCallback, this));
...
void CLImageProc::processImage(const sensor_msgs::msg::Image::SharedPtr msg)
{
    ...
    cl::size_type buffer_size = sizeof(cl_uchar) * width * height * image.channels();
    cl::size_type resize_buffer_size = sizeof(cl_uchar) * resize_width * resize_height * image.channels();
    ...
    std::string filename = "/home/user/DEVELOPMENT/ros_ws/saved_images/2nodes_image_" + std::to_string(image_count_) + ".png";
